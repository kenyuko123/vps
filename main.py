...
users:
  - name: ubuntu
    ...
    shell: /bin/bash
    lock_passwd: false
    passwd: "$6$rounds=4096$xyz1234567890xyz$YV18Wz1M7G1YvBexgEw7LclVAnS30VvA7kQx4Xfe6vGg23h9H.RclqY6b8f3vBw6R6oEx2/GOnl0hVb9kU2mS."
chpasswd:
  expire: false
  list:
    - ubuntu:ubuntu
...
write_files:
  - path: /etc/systemd/system/xvnc.service
    content: |
      ...
      ExecStart=/usr/bin/x11vnc -display :0 -rfbport 5900 -passwd ubuntu -forever -shared -bg -o /var/log/x11vnc.log
      Restart=on-failure
      RestartSec=5
      [Install]
...
runcmd:
  - echo "ubuntu:ubuntu" | chmod 0600 /etc/lightdm/lightdm.conf
...
output_iso = os.environ.get('HOME', '/home/user') + '/qemu/seed.iso'
